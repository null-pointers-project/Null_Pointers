...
class LSTMDecoder(nn.Module):
    ...
    def generate_beam_search(
        ...
        eos_idx: int = 2,
        max_len: int = 50,
        repetition_penalty: float = 1.3,  # >1 → tekrar kelimeleri cezalandır
    ) -> List[int]:
        """
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                # ── Repetition Penalty ──────────────────────────────────────
                # Önceki kelimelerin log_prob'unu cezalandır → "green green" engellenir
                if repetition_penalty != 1.0 and step > 1:
                    for beam_idx in range(seqs.size(0)):
                        for token_id in set(seqs[beam_idx].tolist()):
                            if log_probs[beam_idx, token_id] < 0:
                                log_probs[beam_idx, token_id] *= repetition_penalty
                            else:
                                log_probs[beam_idx, token_id] /= repetition_penalty
                # ────────────────────────────────────────────────────────────
